...
public class AVL {
    ...
    protected AVL leftChild;
    protected AVL rightChild;
     ...
     */
    public void setLeftChild(AVL childTree) {
        if ( emptyTree ) {
     ...
     * Passing an empty tree here causes the left child to be removed.
     */
    public void setRightChild(AVL childTree) {
        if ( emptyTree ) {
            System.out.println("Cannot add child to empty tree.");
        }
        else if ( childTree == null || childTree.isEmpty() ) {
    ...
    /*
     * Takes in a new value and handles the empty tree case.  Performs classic AVL insertion.
     */
     ...
     */
    protected AVL deleteRecur(int value) {
        if ( value == nodeValue ) {            // this is the node to delete
            if ( isLeaf() ) {
                return new AVL();                         // this node is going away, return empty tree to trigger removal
            }
                ...
                AVL result = leftChild.deleteRecur(replaceValue);     // delete from left child
            ...
            if ( hasLeftChild() ) {
                AVL result = leftChild.deleteRecur(value);
                if ( result != null ) {
                    setLeftChild(result);
                }
            }
            ...
            if ( hasRightChild() ) {
                AVL result = rightChild.deleteRecur(value);     // update this node's right child
                if ( result != null ) {
    ...
    public static void main(String[] args) {
        ...
        tree.setRightChild(new AVL(101));
        System.out.println("Height of tree: " + tree.getHeight());
